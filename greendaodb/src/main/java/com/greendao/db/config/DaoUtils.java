...
public class DaoUtils<T> {
    ...
    public DaoUtils(Class<T> pEntityClass, AbstractDao<T, Long> pEntityDao) {
    ...
    public T query() {
        return (T) mDaoSession.queryBuilder(entityClass).unique();
    }
     ...
     * 使用queryBuilder进行查询
     */
    public List<T> queryByQueryBuilder(Property orderDesc) {
        QueryBuilder<T> queryBuilder = mDaoSession.queryBuilder(entityClass);
        return queryBuilder.orderDesc(orderDesc).list();
    }
    /**
     * 使用queryBuilder进行查询
     */
     ...
     * 使用queryBuilder进行查询
     */
    public List<T> queryByQueryBuilder(Property orderDesc, WhereCondition cond, WhereCondition... condMore) {
        QueryBuilder<T> queryBuilder = mDaoSession.queryBuilder(entityClass);
        return queryBuilder.where(cond, condMore).orderDesc(orderDesc).list();
    }
}